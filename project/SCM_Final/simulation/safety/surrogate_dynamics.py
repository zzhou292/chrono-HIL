...
in ``nn_tire_model.py``.  The retained MLP tire surrogates are tiny
(thousands of params), so a pure numpy forward pass on a
``(input_dim, K)`` matrix runs in a few hundred microseconds and matches
the 10 Hz safety-loop budget with plenty of head-room.

Implemented for ``StaticMLP``. Rate variants such as the retained
`vehicle_rate_64_32_lhs` fall back to the scalar CasADi function; that is
slower but correct, and is wired so the same shield code paths work
...
class NumpyTireSurrogate:
    ...
    def __init__(self, nn_model):
        ...
        # The rate-augmented MLP (deployed `vehicle_rate_64_32_lhs`) is a plain
        # tanh MLP just like ``static_mlp`` — only its input feature vector
        # differs (14 inputs: 5 ops + 3 rates + 6 terrain).  It therefore gets
        # the same vectorized numpy forward pass; without it the shield falls to
        # the per-sample CasADi loop below and the MPPI rollout runs at ~0.03x
        # real-time (every sweep run then exceeds its wall-clock timeout).
        self._is_rate_mlp = (nn_model.model_type == 'rate_mlp')

        # Pre-discover layer indices / weight blobs for the fast paths
        if nn_model.model_type in ('static_mlp', 'rate_mlp'):
    ...
    def predict_batch(self, alpha, Fz, u, kappa, terrain_params):
        ...
        if self._fast and self._nn.model_type == 'static_densenet':
            return self._densenet_static_batch(alpha, Fz, u, kappa, sr, n_t,
                                               Kphi, Kc, c, phi, k)
        if self._is_rate_mlp:
            # Match the per-sample loop's phi handling exactly: that loop passes
            # ``terrain_params={'phi': degrees(phi)}`` into ``predict_numeric``,
            # which re-applies ``phi_feature_value`` inside ``predict``.  We
            # reproduce that round-trip so the vectorized path is numerically
            # identical to the scalar fallback it replaces.
            phi_net = self._nn.phi_feature_value(float(np.degrees(phi)))
            return self._rate_mlp_batch(alpha, Fz, u, kappa, n_t,
                                        Kphi, Kc, c, phi_net, k)
    ...
    def _rate_mlp_batch(self, alpha, Fz, u, kappa, n_t,
                        Kphi, Kc, c, phi_net, k):
        """Vectorized forward for the rate-augmented MLP (14 inputs).

        Feature order matches ``RateMLP._build`` in ``nn_tire_model.py``:
            (kappa, alpha, u, Fz, sr, dk, da, du, Kphi, Kc, n, c, phi, k)
        The shield's rollout supplies no steering rate or signal rates, so
        ``sr`` and the three rate channels ``(dk, da, du)`` are zero — exactly
        what the scalar ``predict()`` substitutes when called with
        ``steering_rate=0`` and ``rates=None``.  ``phi_net`` is already in the
        units the checkpoint's scaler expects (see caller).
        """
        K = alpha.size
        zeros = np.zeros(K)
        const_row = lambda v: np.full(K, v)
        X = np.vstack([
            kappa, alpha, u, Fz, zeros,            # kappa, alpha, u, Fz, sr
            zeros, zeros, zeros,                   # dk, da, du
            const_row(Kphi), const_row(Kc), np.full(K, n_t),
            const_row(c), const_row(phi_net), const_row(k),
        ])  # (14, K)
        H = (X - self._X_mean[:, None]) / self._X_scale[:, None]
        for i in self._layer_idx:
            W = self._w[f'layers.{i}.weight']
