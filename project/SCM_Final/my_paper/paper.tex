...
\paragraph{Scope: capability, not a human-subjects study.}
We report the HIL pipeline as a \emph{demonstrated capability} rather than as a
quantitative human-subjects study. A controlled study --- multiple operators,
repeated trials, and the attendant human-subjects protocol --- is out of scope
for this systems paper, and a single-operator pilot under latency is too
high-variance to support a quantitative filter claim: even an experienced
operator cannot reproduce a round, so raw per-operator collision counts are a
noisy estimator of a filter's effect. We therefore make the quantitative safety
claim with the deterministic \emph{counterfactual replay} of
Sec.~\ref{sec:safety_convoy}, which removes inter-operator variance entirely by
replaying one command trace filter-off versus DOB-CBF-on on the \emph{identical}
scenario. The trace reported there is a synthetic aggressive-operator proxy
(constant throttle into the hazard, the autonomous stand-in for an operator who
fails to react under latency); because the simulator is bit-for-bit
deterministic, the same machinery replays a \emph{recorded} G29 trace unchanged.
A full human-subjects evaluation, for which the pipeline of
Fig.~\ref{fig:hil_hud} is built, is left to future work.
