...
are replayed, so blind-vs-aware is paired. Over $30$ cells per variant ($5$
convoy scenarios $\times$ two delays $\times$ three intents), both filters reduce
collisions substantially relative to no filter ($0.87$), but the delay-aware filter
is strictly better: a $0.13$ collision rate vs the delay-blind $0.23$, at nearly
twice the clearance margin ($+2.12$ vs $+1.10\,$m). Crucially the gap
\emph{grows with delay} (Fig.~\ref{fig:latency_ablation}): at
$\Delta_\mathrm{cmd}\!=\!0.15\,$s the two tie on collisions ($0.27$) but the
aware filter already holds more margin ($+1.67$ vs $+0.97\,$m), while at
$0.30\,$s delay-awareness removes the residual collisions the
delay-blind filter still suffers ($0.20\!\to\!0.00$). This supports the
\emph{latency-aware} claim specifically, rather than only showing generic
latency robustness.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{latency_awareness_ablation.png}
  \caption{Latency-awareness ablation. The same adversarial intent is replayed
    with the command path delayed by $\Delta_\mathrm{cmd}$, the DOB-CBF filter
    either \emph{told} that delay (aware) or not (blind). A filter helps at both
    delays, but knowing the delay matters more as the delay grows: at $0.30\,$s
    it removes the residual collisions the delay-blind filter still suffers.
    $30$ deterministic cells per bar.}
