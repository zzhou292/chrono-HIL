...
\begin{figure*}[!t]
  ...
  \caption{Open-loop NMPC prediction versus the Chrono plant for a
    ...
    launch time; dots mark the launch point) branches off the actual
    plant trace (bold black). \textbf{Left:} longitudinal speed $u$ ---
    the horizons run \emph{above} the plant and the gap widens along the
    horizon, the $F_x$ motion-resistance bias the throttle disturbance
    observer absorbs online. \textbf{Right:} heading $\psi$ --- the
    horizons hug the plant, so the path geometry (lateral $F_y$) is
    predicted accurately. Aggregate drifts across the 18-run matrix are
    quoted in the text ($\le 0.15\,$rad heading over $4\,$s versus a
    speed bias growing to ${\approx}1\,$m/s on firm sand).}
