...
filter, rather than committing to the operator's wide lateral bypass, cut
throttle to a crawl and re-centred the truck ($|\Delta\delta|\!=\!0.23$,
$|\Delta\tau|\!=\!0.37$), pinning it against the stalled vehicle's corner where
the operator had instead kept speed and passed with $1.0\,$m clearance. On
genuine human intent the filter thus restores large clearance margins on the
cases it is designed for, without fighting a capable driver. (Single operator; a
multi-operator study is left to future work.)
...
\begin{figure}[!t]
  ...
  \caption{Counterfactual replay on ten \emph{recorded human} G29 drives (five
    ...
    case where the filter over-braked and re-centred, pinning the truck against
    the stalled vehicle instead of swerving wide as the operator did.}
  \label{fig:teleop_cf}
\end{figure}
