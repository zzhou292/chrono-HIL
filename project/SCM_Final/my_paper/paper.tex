...
new flavors plug in without touching consumers. A differentiable
...
stack cuts median RMS crosstrack error by $35$--$43\,\%$ over
...
shields cut collisions by more than 90\,\% under the learned 5G
...
\begin{enumerate}
  \item A modular framework (Section~\ref{sec:arch}) in which six
        ...
        test rejects any flavor that drifts from its declared API,
  ...
  \item A two-layer obstacle-avoidance stack
        ...
        MPPI predictive shield with seeded recovery trajectories, and
...
new flavor is a one-line registration with no edits to the consumers.
...
safety shield re-conditions itself without a separate high-priority
...
\begin{table}[!t]
  ...
  \caption{The six swap points of the framework. Each role is defined
    ...
    test instantiates one of each shipped flavor and checks it against
  ...
  \begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{Role} & \textbf{Protocol method} & \textbf{Shipped flavors}\\
    \midrule
...
aggregated training table contains roughly $1.85\,\mathrm{M}$ rows of
...
\begin{table}[!t]
  ...
  \caption{Tire-model benchmark over the full 1215-run matrix (405 runs
    ...
    $42$--$47\,\%$ below the analytical baselines; the per-model
    \emph{medians} are close ($0.13$--$0.15\,\mathrm{m}$), so the NN's
    gain is in the tail --- the Pacejka and TMeasy rows carry large
    right-skewed standard deviations ($0.42$--$0.46$ vs.\ $0.19$) from
    off-nominal dirt scenarios (Fig.~\ref{fig:tires_static_heatmap})
    that the NN suppresses. All models retain similar speed because the
    curvature-derived speed profile, not the tire force model, is the
    binding speed constraint in this regime. The NN's higher solve time
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Pacejka                       & $0.36 \pm 0.46$ & $0.66$ & $4.2$\\
    TMeasy                        & $0.33 \pm 0.42$ & $0.66$ & $4.6$\\
    \textbf{Vehicle NN rate-MLP}  & $\mathbf{0.19 \pm 0.19}$ & $0.66$ & $14.0$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[!t]
  ...
  \caption{Closed-loop RMS CTE per (terrain, path) cell across the
    ...
    track well on clay and sand but blow out on dirt/sinusoidal
    ...
    matching the calibrated baselines on the easy cells. This per-scenario
...
(median $0.17$ vs.\ $0.15\,\mathrm{m}$, an $18\,\mathrm{mm}$ gap whose
$95\,\%$ bootstrap confidence intervals, $[0.14,0.18]$ and
$[0.13,0.18]\,\mathrm{m}$, overlap --- statistically indistinguishable)
while requiring no prior terrain knowledge, and both NN rows reduce
median RMS CTE by $35$--$43\,\%$ relative to the SCM-calibrated Pacejka
...
strongly right-skewed by a handful of high-error scenarios (mostly
...
\begin{table}[!t]
  ...
  \caption{Live-estimator-conditioned tire-model benchmark, full
    ...
    strongly right-skewed by a handful of high-error dirt scenarios
    ...
    (median $0.17$ vs.\ $0.15\,\mathrm{m}$, an $18\,\mathrm{mm}$ gap with
    overlapping $95\,\%$ bootstrap CIs). Both NN rows reduce median RMS
    CTE by $35$--$43\,\%$ relative to the \emph{SCM-calibrated}
  ...
  \begin{tabular}{lccc}
    ...
    Pacejka (static)            & $0.26$ & ---       & ---\\
    TMeasy (static)             & $0.27$ & ---       & ---\\
    NN rate-MLP (static)        & $0.15$ & $-42\,\%$ & $-43\,\%$\\
    \textbf{NN rate-MLP $+$ estimator} & $\mathbf{0.17}$ & $\mathbf{-35\,\%}$ & $\mathbf{-37\,\%}$\\
    \bottomrule
  \end{tabular}
\end{table}
...
dirt prior gives RMS CTE $1.62\,\mathrm{m}$, the estimator-on variant
(the deployed Fused-UKF, Sec.~\ref{sec:ukf}) adapts from the same dirt
initialisation and recovers to $0.48\,\mathrm{m}$, and the
matched-prior oracle gives $0.42\,\mathrm{m}$ -- the estimator closes
...
\begin{table}[!t]
  ...
  \caption{Wrong-prior ablation: RMS CTE (m) when the controller's
    ...
    wrong priors are both fine on sand because dirt is closer to sand
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Matched static prior (oracle) & $0.417$ & $0.062$ & $0.239$\\
    Wrong (dirt) static prior     & $1.620$ & $0.057$ & $0.838$\\
    \textbf{Estimator-on (dirt $\to$ adapt)} & $\mathbf{0.475}$ & $\mathbf{0.060}$ & $\mathbf{0.268}$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[!t]
  ...
  \caption{Same benchmark, RMS CTE distribution per variant.
    ...
    carry long right tails driven by a handful of dirt cases at higher
...
``rig wins everything'' result narrows substantially:

\begin{itemize}
\item Static MLPs: rig still wins decisively
  ($0.085\,\mathrm{m}$ vs $0.112\,\mathrm{m}$, 24\,\%).
\item Rate MLPs (32-16): rig and vehicle tie at $0.090\,\mathrm{m}$.
...
\begin{figure}[!t]
  ...
  \caption{Aggregate metrics over the 432-run controlled sweep, six
    ...
    surrogate. The pre-retrain ``rig wins everything'' result narrows
...
the catastrophic case they tie at $0.185\,\mathrm{m}$.  The
rate-augmented variants pay for their force-RMSE deficit with a
...
\begin{figure}[!t]
  ...
  \caption{One representative scenario from the four-way comparison
    ...
    envelope most tightly. The deployed default still wins on RMS
    CTE but pays for that smoothness with the largest force-RMSE.}
...
clay--dirt--sand terrain manifold. The controller and safety shield
...
\begin{figure}[!t]
  ...
  \caption{Open-loop excitation vs.\ closed-loop tracking, all four
    ...
    $0.44\!\to\!0.09$ --- \emph{observability}-limited, a real maneuver
    fixes it --- whereas the Bekker-UKF stays $\approx\!0.47$ in both
    (\emph{model}-limited: its analytical contact law is wrong on sand).
    The proprioceptive MLP is excitation-agnostic ($0.04\!\to\!0.00$),
    and the deployed Fused-UKF is strong in both ($\approx\!0.01$),
...
manoeuvre axes — steering amplitude $0.20\text{--}0.65\,$rad,
...
a \emph{scripted} $0.6\,$rad steering manoeuvre the same delta grows to
...
scripted excitation manoeuvre against the same soil under closed-loop
tracking. The force-only NN-UKF goes from $0.09$ (open-loop) to $0.44$
(closed-loop): it is observability-limited, and a real excitation
manoeuvre \emph{fixes} it. The Bekker-UKF stays bad in both
($0.48\!\to\!0.47$): it is model-limited --- the analytical Bekker law
is simply wrong on sand, and no amount of excitation rescues it. The
proprioceptive MLP, which reads vibration and is excitation-agnostic,
is strong in both ($0.00\!\to\!0.04$), and the Fused-UKF inherits that
...
\emph{no} dedicated excitation manoeuvre at runtime.
...
\begin{table}[!t]
  ...
  \caption{Asymmetric throttle disturbance observer ablation.
    64 runs per variant. Speed ratio is reported against the
    \emph{curvature-limited reference profile} (mean
    $u_\mathrm{meas}/\mathrm{mean}\,v_\mathrm{ref}(s)$) rather than the
    ...
    asked to track. The observer lifts achieved speed by approximately
    $5\,\%$ at a small ($\sim$3\,cm) increase in tracking error.}
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    DOB off & $0.153 \pm 0.131$ & $0.63$ & $4.24$\\
    DOB on  & $0.184 \pm 0.166$ & $\mathbf{0.66}$ & $\mathbf{4.42}$\\
    \bottomrule
  \end{tabular}
\end{table}
...
DOB-CBF flavor solves a single-step minimum-deviation QP around the
...
an intrusiveness signal. The MPPI flavor runs at the controller rate,
...
surrogate; (ii) the MPPI predictive shield; (iii) a gradient-based
...
reading is that the DOB-CBF and MPPI shields cut collisions relative to the
...
already safe), corroborating the autonomous shield-only sweep of
...
stands in for the operator, isolating the shield as the sole collision
avoider. This
...
shield-only outcome. Because the planner's reference here is
...
shield must pull the vehicle off that obstacle-blind reference to stay
...
\begin{table}[!t]
  ...
  \caption{Autonomous shield-only validation (planner blind to
    ...
    DOB-CBF is the least intrusive shield (lowest intervention rate),
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    None         & $3.06$ & $-1.82$ & --- & $0.29$\\
    \textbf{DOB-CBF} & $0.51$ & $+0.21$ & $\mathbf{52}$ & $2.54$\\
    \textbf{MPPI}    & $\mathbf{0.08}$ & $\mathbf{+2.27}$ & $89$ & $1.13$\\
    NMPC (SLSQP) & $0.52$ & $+1.19$ & $83$ & $7.38$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure*}[!t]
  ...
  \caption{Autonomous shield-only validation. Mean unique obstacles hit
...
across all four shield flavours under both planner-aware and
planner-blind modes.
Barriers alone (no-shield, planner-aware) cut collisions by roughly
$5.6\times$ relative to the planner-blind baseline ($3.06\to0.55$ per
run), and combining the barrier with the DOB-CBF or MPPI shield drives
collisions to near zero ($0.17$ and $0.07$ per run, respectively).

\begin{table}[!t]
  \renewcommand{\arraystretch}{1.10}
  \caption{Planner-aware vs.\ planner-blind safety sweep. 3240 runs.
    The two-layer stack (barrier $+$ shield) drives collisions to near
    zero ($\le 0.17$/run) for both DOB-CBF and MPPI shields.}
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    None, blind          & $3.11$ & $-1.83$ & --- & $0.33$\\
    None, aware          & $0.55$ & $+0.36$ & --- & $2.48$\\
    DOB-CBF, blind       & $0.46$ & $+0.30$ & $52$ & $2.52$\\
    \textbf{DOB-CBF, aware} & $\mathbf{0.17}$ & $+1.31$ & $33$ & $2.97$\\
    MPPI, blind          & $0.09$ & $+2.07$ & $89$ & $1.34$\\
    \textbf{MPPI, aware}    & $\mathbf{0.07}$ & $+1.91$ & $85$ & $2.85$\\
    NMPC, blind          & $0.50$ & $+1.17$ & $84$ & $6.90$\\
    NMPC, aware          & $0.28$ & $+1.44$ & $80$ & $7.70$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!t]
  ...
  \begin{tabular}{lcccc}
    ...
    \textbf{Variant} & \textbf{Coll./run} & \textbf{Clearance (m)} & \textbf{Interv. (\%)} & \textbf{CTE (m)}\\
    \midrule
    No filter        & $3.11$ & $-1.83$ & --- & $0.35$\\
    DOB-CBF, no NN   & $0.86$ & $-0.22$ & $46$ & $2.00$\\
    \textbf{DOB-CBF, with NN} & $\mathbf{0.54}$ & $\mathbf{+0.06}$ & $51$ & $2.29$\\
    \bottomrule
  \end{tabular}
\end{table}
...
increases the collision rate by approximately 30$\times$ and drives
...
\begin{table}[!t]
  ...
  \begin{tabular}{lcccc}
    ...
    \textbf{Variant} & \textbf{Coll./run} & \textbf{Clearance (m)} & \textbf{Interv. (\%)} & \textbf{CTE (m)}\\
    \midrule
    MPPI, no seeds   & $2.19$ & $-1.12$ & $87$ & $1.26$\\
    \textbf{MPPI, with seeds} & $\mathbf{0.07}$ & $\mathbf{+2.06}$ & $89$ & $1.10$\\
    \bottomrule
  \end{tabular}
\end{table}
...
MPPI shield always on, varying only the planner's tire model (the
...
\begin{table}[!t]
  ...
  \caption{Autonomous obstacle avoidance, planner tire model swept
    under a fixed MPPI shield (analytical planners SCM-calibrated).
...
\begin{figure*}[!t]
  ...
  \caption{Autonomous obstacle avoidance, planner tire model swept
    under a fixed MPPI shield. Mean unique obstacles hit per run
    ...
    the shield, not the planner-tire choice, dominates the safety
...
report the result over 1215 trials. Both shield variants reduce
collisions by more than 90\,\% relative to the no-shield
baseline (DOB-CBF by $99.7\,\%$, MPPI by $96\,\%$).
...
\begin{table}[!t]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    None      & $2.43$ & $-1.40$ & ---\\
    DOB-CBF   & $\mathbf{0.01}$ & $+2.51$ & $53$\\
    MPPI      & $0.09$ & $\mathbf{+3.26}$ & $86$\\
    \bottomrule
  \end{tabular}
\end{table}
...
fires roughly $1.6\text{--}1.8\,\mathrm{s}$ before impact; on firm
...
Reproducibility is enforced through a handful of architectural
...
re-condition an unstructured surrogate. A drop-in adaptation of their
...
\emph{SCM-calibrated} analytical baselines by $35$--$43\,\%$ in the
...
blow out on off-nominal dirt. The asymmetric
throttle disturbance observer lifts achieved speed by approximately
$5\,\%$ ($4.24\!\to\!4.42\,\mathrm{m/s}$, $0.63\!\to\!0.66$ of the
curvature-limited profile) at a small (${\sim}3\,\mathrm{cm}$) tracking
cost. The two-layer planner-aware stack drives collisions to near zero
($\le 0.17$/run) when paired with either DOB-CBF or MPPI as the
downstream shield, the MPPI seed trajectories are essential rather
than auxiliary, the DOB-CBF filter benefits substantially from
re-using the neural surrogate, and both shields cut collisions by
...
learned surrogate versus analytical tire model, shield versus no shield,
...
SCM-calibrated analytical model rather than a rigid-road strawman, and
...
$n$-estimator that re-conditions it, swappable safety shields, and an
...
$\approx\!20\,\mathrm{mm}$ median RMS crosstrack error of the
...
range at $4.5\,\%$ median error; both safety shields cut collisions by
