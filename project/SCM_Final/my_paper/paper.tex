...
(scope and limits in Sec.~\ref{sec:limitations}). The MPPI variant runs at the controller rate,
...
\begin{table}[!t]
  ...
  \begin{tabular}{lccccc}
    ...
    \textbf{Variant} & \textbf{Coll./run} & \textbf{Clearance (m)} & \textbf{Interv. (\%)} & $\mathbf{|\Delta\delta|}$ & $\mathbf{|\Delta\tau|}$\\
    \midrule
    MPPI, no seeds   & $2.18$ & $-1.05$ & $86$ & $0.22$ & $0.25$\\
    \textbf{MPPI, with seeds} & $\mathbf{0.08}$ & $\mathbf{+2.03}$ & $88$ & $0.46$ & $0.61$\\
    \bottomrule
  \end{tabular}
\end{table}
