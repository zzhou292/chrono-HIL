...
with a deterministic counterfactual replay attributing the prevented
collisions causally to the filter on the operator's own intent ---
...
\begin{enumerate}
  ...
  \item A two-layer obstacle-avoidance stack
        ...
        \emph{counterfactual replay} of the operator's own command trace
        (filter off vs.\ on), giving a causal collisions-prevented
        measurement that is robust to inter-operator variance.
...
that removes inter-operator variance. Because the simulator is
deterministic, the recorded operator command trace can additionally be
replayed with the filter off and through each filter on the identical
scenario, yielding a per-operator \emph{counterfactual} harm-prevented
measurement --- the collisions averted and clearance recovered on the
operator's own intent --- with no inter-operator variance to average out
and no crosstrack-tracking proxy.
...
\begin{figure*}[!t]
  ...
  \caption{\textbf{[PLACEHOLDER --- pending HIL runs.]} Human-in-the-loop
    ...
    intervention rate, mean throttle/steering correction, and speed
    retention. The operator is avoiding obstacles toward a goal, not
    tracking a reference path, so crosstrack error is not reported. To
...
cost a teleoperator would feel. MPPI is the safest (fewest collisions,
largest clearance) but the most intrusive: it intervenes on $88\,\%$ of
steps and applies the largest throttle correction as it brakes hardest.
DOB-CBF intervenes least often ($52\,\%$) with the smallest throttle
correction --- the minimum-deviation, intent-preserving behaviour that
motivates it as the default HIL filter.
...
\begin{table}[!t]
  ...
  \caption{Autonomous safety-filter-only validation (planner blind to
    ...
    Sec.~\ref{sec:safety_hil}. $|\Delta\delta|$, $|\Delta\tau|$ are the mean
    absolute steering and throttle command corrections the filter applies.}
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    None         & $3.05$ & $-1.83$ & --- & --- & ---\\
    \textbf{DOB-CBF} & $0.46$ & $+0.26$ & $\mathbf{52}$ & $0.68$ & $\mathbf{0.27}$\\
    \textbf{MPPI}    & $\mathbf{0.10}$ & $\mathbf{+2.17}$ & $88$ & $0.53$ & $0.66$\\
    NMPC (SLSQP) & $0.45$ & $+1.13$ & $84$ & $0.88$ & $0.42$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!t]
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    None, blind          & $3.01$ & $-1.83$ & --- & --- & ---\\
    None, aware          & $0.49$ & $+0.39$ & --- & --- & ---\\
    DOB-CBF, blind       & $0.50$ & $+0.22$ & $52$ & $0.69$ & $0.27$\\
    \textbf{DOB-CBF, aware} & $\mathbf{0.20}$ & $+1.24$ & $35$ & $0.35$ & $0.24$\\
    MPPI, blind          & $0.07$ & $+2.23$ & $88$ & $0.50$ & $0.65$\\
    \textbf{MPPI, aware}    & $\mathbf{0.04}$ & $+2.07$ & $84$ & $0.31$ & $0.54$\\
    NMPC, blind          & $0.51$ & $+1.11$ & $84$ & $0.91$ & $0.41$\\
    NMPC, aware          & $0.26$ & $+1.35$ & $80$ & $0.64$ & $0.34$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!t]
  ...
  \begin{tabular}{lccccc}
    ...
    \textbf{Variant} & \textbf{Coll./run} & \textbf{Clearance (m)} & \textbf{Interv. (\%)} & $\mathbf{|\Delta\delta|}$ & $\mathbf{|\Delta\tau|}$\\
    \midrule
    None (no filter) & $3.05$ & $-1.83$ & --- & --- & ---\\
    DOB-CBF, no NN   & $0.81$ & $-0.21$ & $46$ & $0.71$ & $0.26$\\
    \textbf{DOB-CBF, NN} & $\mathbf{0.48}$ & $\mathbf{+0.12}$ & $50$ & $0.69$ & $0.26$\\
    \bottomrule
  \end{tabular}
\end{table}
...
These scenarios also enable a \emph{counterfactual} measurement that removes
the operator-variance confound entirely. Because the simulator is bit-for-bit
deterministic, a single operator command trace can be replayed through the
identical scenario with the filter off and with each filter, so every outcome
difference is attributable to the filter alone --- no inter-operator averaging
and no path-tracking proxy. Table~\ref{tab:convoy_cf} and
Fig.~\ref{fig:convoy_cf} report a representative aggressive intent (constant
forward throttle into the hazard --- the autonomous proxy for an operator who
fails to react under latency; recorded operator traces from the HIL rounds of
Sec.~\ref{sec:safety_hil} replay through the same machinery) across three
convoy scenarios $\times$ two command delays ($0$, $0.30\,$s), six cells per
filter. The unfiltered intent collides in all six cells; both filters prevent
every collision. MPPI opens the larger clearance ($+5.6$ vs $+3.5\,$m) by
braking ($|\Delta\tau|=0.27$), whereas DOB-CBF avoids almost entirely by
steering ($|\Delta\tau|\approx0$, $|\Delta\delta|=0.26$) --- the same
minimum-deviation, intent-preserving signature seen in the autonomous sweep,
now established causally on identical operator intent.

\begin{table}[!t]
  \renewcommand{\arraystretch}{1.10}
  \caption{Counterfactual convoy replay: one aggressive operator intent
    replayed filter-off vs each filter across 3 traffic scenarios
    (lead-brake, cut-in, stalled) $\times$ 2 command delays (6 cells).
    The deterministic sim makes harm-prevented causal. $|\Delta\delta|$,
    $|\Delta\tau|$ are the mean steering/throttle command corrections.}
  \label{tab:convoy_cf}
  \centering
  \setlength{\tabcolsep}{3pt}
  \footnotesize
  \begin{tabular}{lccccc}
    \toprule
    \textbf{Filter} & \textbf{Coll.} & \textbf{Prevented} & \textbf{Clear.\,(m)} & $\mathbf{|\Delta\delta|}$ & $\mathbf{|\Delta\tau|}$\\
    \midrule
    None      & $6/6$ & --- & $-1.43$ & --- & ---\\
    \textbf{DOB-CBF} & $\mathbf{0/6}$ & $\mathbf{6/6}$ & $+3.46$ & $0.26$ & $0.00$\\
    \textbf{MPPI}    & $\mathbf{0/6}$ & $\mathbf{6/6}$ & $\mathbf{+5.62}$ & $0.29$ & $0.27$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{convoy_counterfactual.png}
  \caption{Counterfactual convoy replay (Table~\ref{tab:convoy_cf}). The same
    operator intent collides in every unfiltered cell (left); both filters
    prevent all collisions and open positive clearance (centre); DOB-CBF does
    so by steering with negligible throttle correction, MPPI by braking
    (right).}
  \label{fig:convoy_cf}
\end{figure}

\subsection{Real-time scaling with obstacle count}
\label{sec:safety_scaling}

The two filters scale very differently with the number of in-range obstacles
$N$ --- and since a rock and a traffic vehicle are identical $(x,y,r)$ entries
to the filter, $N$ counts both jointly. Timing each filter's solve in
isolation against the $100\,$ms ($10\,$Hz) shield budget
(Fig.~\ref{fig:safety_scaling}): the DOB-CBF QP costs $0.25\,$ms at $N=0$ and
only $3.5\,$ms at $N=128$, so it never approaches the budget and has no
practical obstacle-count limit. MPPI is instead dominated by its sampled
rollout --- $\approx\!85\,$ms at the deployed $K=384$ samples \emph{regardless
of $N$}, with each obstacle adding only $\sim\!0.3\,$ms --- so it crosses the
budget near $N=64$ (a capacity of $\approx\!32$). Its real-time ceiling is set
by the sample count, not the scene: solve time is linear in $K$ ($85\,$ms at
$K=384$, over budget by $K=512$, $194\,$ms at $K=1024$). This is a concrete
guide for the swappable design: DOB-CBF for dense scenes at negligible cost,
MPPI when predictive avoidance is worth the fixed rollout, with $K$ chosen
against the control-rate budget. Crucially, that budget is the only thing
forcing $K$ down: replaying the aggressive intent through the convoy scenarios
at each $K$, MPPI is \emph{collision-free across $K\!\in\![64,512]$} with
clearance growing only marginally ($5.3\!\to\!5.9\,$m) --- the hand-crafted
seed trajectories (Sec.~\ref{sec:safety_seeds}) supply the evasive options
regardless of the random sample count. The deployed $K=384$ ($90\,$ms) is thus
conservative: $K=64$ is equally collision-free at $30\,$ms ($3\times$ the
real-time margin), while $K=512$ already exceeds the budget. Safety is flat in
$K$ and cost is linear, so $K$ is a pure real-time knob here, not a safety one.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{safety_filter_scaling.png}
  \caption{Safety-filter solve time vs.\ obstacle count $N$ (left) and MPPI
    sample count $K$ (right), against the $100\,$ms / $10\,$Hz shield budget
    (dashed). DOB-CBF stays $\le\!3.5\,$ms out to $N=128$; MPPI is
    rollout-bound --- flat in $N$, linear in $K$. Standalone solve timing,
    single-thread BLAS.}
  \label{fig:safety_scaling}
\end{figure}
...
baseline (DOB-CBF by $91.4\,\%$, MPPI by $96.8\,\%$). MPPI is the more
effective shield against the latency outages, whereas DOB-CBF trades some
of that margin for the minimum-deviation, intent-preserving behaviour that
suits the human-in-the-loop case (Sec.~\ref{sec:safety}).
...
\begin{table}[!t]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    None      & $2.80$ & $-1.44$ & ---\\
    DOB-CBF   & $0.24$ & $+0.78$ & $49$\\
    MPPI      & $\mathbf{0.09}$ & $\mathbf{+2.71}$ & $88$\\
    \bottomrule
  \end{tabular}
\end{table}
...
The safety filters are fundamentally \emph{forward}-collision-avoidance: their
...
speed --- the MPPI filter is not yet rear-aware, and this behaviour has so far
been validated only on a single rear-approach scenario in simulation. Lane-
change escape and a predictive rear cost in MPPI are the natural extensions.
Estimating the full soil vector online, hardware transfer, and the
human-subject teleoperation study the HIL protocol is built for are the
principal directions for future work.
