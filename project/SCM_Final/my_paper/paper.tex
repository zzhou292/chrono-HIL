...
deliberately routed through the obstacles, the deviation column (the
crosstrack error from that obstacle-blind reference) does not measure
tracking quality: it measures how far an active safety filter must pull
the vehicle off the reference to stay safe, and therefore reads as an
intrusiveness proxy. A larger deviation for DOB-CBF and NMPC than for MPPI
is consistent with their higher per-step deviation, not with worse tracking.
...
\begin{table}[!t]
  ...
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Filter} & \textbf{Coll./run} & \textbf{Clearance (m)} & \textbf{Interv. (\%)} & \textbf{Deviation (m)}\\
    \midrule
    None         & $3.05$ & $-1.83$ & --- & $0.25$\\
...
\begin{table}[!t]
  ...
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Variant} & \textbf{Coll./run} & \textbf{Clearance (m)} & \textbf{Interv. (\%)} & \textbf{Deviation (m)}\\
    \midrule
