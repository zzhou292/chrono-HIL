...
barriers with a swappable downstream safety filter; the shipped filter
is an intent-preserving DOB-CBF, the minimum-deviation screen suited to
human teleoperation. A parallel terrain- and latency-aware forward
...
recovers part of the soft-soil speed gap at a small tracking cost, the
DOB-CBF filter sharply cuts collisions under the learned 5G profile ---
a deterministic counterfactual replay over a population of adversarial intents
causally attributes the averted collisions to the filter ($31$ of $39$) ---
and the warning predicts stopping distance to within
...
\begin{enumerate}
  ...
  \item A two-layer obstacle-avoidance stack
        ...
        barriers with a swappable downstream safety filter; the shipped
        instance is an intent-preserving DOB-CBF-QP inspired
        by~\cite{Zhang25}, with physical steering/throttle rate limits
        baked into the QP. Its effect is quantified by a deterministic
        \emph{counterfactual replay} --- replaying a fixed command trace
        (filter off vs.\ on) on the identical scenario --- giving a causal,
        exactly reproducible collisions-prevented measurement over a
        population of adversarial intents.
...
\begin{table}[!t]
  ...
  \caption{Tire-model benchmark over the full 1215-run matrix (405 runs
    ...
    reflects the embedded surrogate evaluation; the reported solve times are
    representative single-controller timings (absolute values rise under the
    parallel sweep).}
...
$\times$ $5$ seeds, $20$ runs per variant).
...
shipped DOB-CBF safety filter, which reads the surrogate as a static
...
rate limit baked into the QP and enforced at the physics step --- so
...
DOB-CBF is the shipped instance: it is the only candidate that screens
...
\paragraph{Scope.}
The HIL pipeline is included to show the framework drives a real human operator
...
\emph{identical} scenario, so the harm prevented is causal and exactly
reproducible. The traces used are adversarial ``drive-into-the-hazard'' intents
--- a worst-case stress test of the filter --- and, because the simulator is
bit-for-bit deterministic, the same machinery replays a recorded G29 trace
unchanged.
...
behaviour that motivates it as the shipped safety filter.
...
\begin{table}[!t]
  ...
  \caption{Autonomous safety-filter-only validation (planner blind to
    ...
    as Table~\ref{tab:safety_aware}, blind rows only. DOB-CBF, the shipped
...
paper's headline safety result. Because the simulator is bit-for-bit
deterministic, a fixed command trace is replayed through the \emph{identical}
scenario with the filter off and on, so every outcome difference is attributable
to the filter alone --- a causal, exactly reproducible harm-prevented measure
...
($0.4$, $0.6$, $0.8$), a worst-case stand-in for an inattentive or
...
--- $45$ deterministic cells per filter. The unfiltered intent collides in $40$
of $45$ cells; DOB-CBF \emph{causally prevents $31$ of the $39$ matched baseline
collisions}, cutting the collision rate from $89\%$ to $20\%$ and turning a mean
...
unchanged, so the measure carries over directly to logged human intent.
...
\begin{table}[!t]
  ...
  \caption{Counterfactual convoy replay: a population of adversarial intents
    ...
    ($45$ deterministic cells per filter). The bit-for-bit deterministic sim
    makes harm-prevented causal. $|\Delta\delta|$, $|\Delta\tau|$ are the mean
...
are replayed, so blind-vs-aware is causal. Over $30$ cells per variant ($5$
convoy scenarios $\times$ two delays $\times$ three intents), both filters cut
collisions sharply relative to no filter ($0.87$), but the delay-aware filter
...
$0.30\,$s delay-awareness \emph{eliminates} the residual collisions the
delay-blind filter still suffers ($0.20\!\to\!0.00$). This substantiates the
\emph{latency-aware} claim specifically, not merely a latency-robust one.
...
reproduced by \texttt{benchmarking/integrated\_hero\_run.py}.
...
population of adversarial intents attributes the averted collisions causally to
the filter ($31$ of $39$, collision rate $89\%\!\to\!20\%$) --- a causal,
exactly reproducible safety measure that needs no human trials. The forward
...
counterfactual causally attributing $31$ of $39$ averted collisions to it;
...
the automated commands because they require a human operator; they
