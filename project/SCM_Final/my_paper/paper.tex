...
The human-driven rounds are the realism source for the counterfactual
replay (Sec.~\ref{sec:safety_convoy}); the autonomous protocol of the following
...
Teleoperation under 5G latency over rough, obstacle-strewn terrain produces
\emph{high-variance} command streams: even an experienced operator cannot
reproduce a round, so raw per-operator collision counts are a noisy estimator
of a filter's effect. Rather than average over that variance, we make the
quantitative safety claim with the deterministic
\emph{counterfactual replay} of Sec.~\ref{sec:safety_convoy}, which
replays each recorded operator intent filter-off versus DOB-CBF-on on the
\emph{identical} scenario and so isolates the filter's causal effect with zero
inter-operator variance. The live human rounds
(\texttt{human\_delay\_compensation\_rounds.py}) serve as the realism source
for those traces and a qualitative demonstration that the intent-preserving
filter is comfortable to drive behind; the autonomous sweeps of
Sec.~\ref{sec:safety_blind} supply the large, fully repeatable matrix.
...
behaviour that motivates it as the shipped HIL filter.
...
\begin{table}[!t]
  ...
  \caption{Autonomous safety-filter-only validation (planner blind to
    ...
    rate. $|\Delta\delta|$, $|\Delta\tau|$ are the mean absolute steering and
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    None         & $3.07$ & $-1.81$ & --- & --- & ---\\
    \textbf{DOB-CBF} & $\mathbf{0.66}$ & $\mathbf{+0.50}$ & $57$ & $0.28$ & $0.39$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!t]
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    None, blind          & $2.99$ & $-1.81$ & --- & --- & ---\\
    None, aware          & $0.52$ & $+0.41$ & --- & --- & ---\\
    DOB-CBF, blind       & $0.66$ & $+0.40$ & $56$ & $0.29$ & $0.36$\\
    \textbf{DOB-CBF, aware} & $\mathbf{0.13}$ & $\mathbf{+2.00}$ & $40$ & $0.14$ & $0.31$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!t]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Pacejka              & $0.64$ & $+0.81$ & $0.77$\\
    TMeasy               & $0.67$ & $+0.77$ & $0.84$\\
    NN rate-MLP          & $0.61$ & $+0.73$ & $0.90$\\
    \bottomrule
  \end{tabular}
\end{table}
...
paper's headline safety result, because it removes the operator-variance
confound entirely. Teleoperation under latency is intrinsically
high-variance --- the same driver cannot reproduce a round --- so averaging
raw human outcomes is a poor estimator of a filter's value. Instead, because
the simulator is bit-for-bit deterministic, a single recorded command trace is
replayed through the \emph{identical} scenario with the filter off and on, and
every outcome difference is attributable to the filter alone: no inter-operator
averaging, no path-tracking proxy, and the operator's own (imperfect) intent as
the input. Table~\ref{tab:convoy_cf} reports a
representative aggressive intent (constant forward throttle into the hazard ---
the autonomous proxy for an operator who fails to react under latency; recorded
HIL traces replay through the same machinery) across five convoy scenarios
(lead-brake, 3- and 5-vehicle convoys, rear-approach, stalled) $\times$ three
command delays ($0$, $0.15$, $0.30\,$s), $15$ cells per filter. The unfiltered
intent collides in $12$ of $15$ cells; DOB-CBF \emph{causally prevents $10$ of
those $12$ collisions} ($2/15$ remaining), turning a mean clearance of
$-1.42\,$m into $+1.82\,$m. It does so at small command deviation
($|\Delta\delta|=0.13$, $|\Delta\tau|=0.26$), mixing steering with braking ---
the obstacle-type-aware QP steers around static rocks but is willing to brake
behind a vehicle --- the minimum-deviation, intent-preserving signature,
established causally on identical operator intent.

\begin{table}[!t]
  \renewcommand{\arraystretch}{1.10}
  \caption{Counterfactual convoy replay: one aggressive operator intent
    replayed filter-off vs DOB-CBF across 5 traffic scenarios
    $\times$ 3 command delays (15 cells). The deterministic sim makes
    harm-prevented causal and free of inter-operator variance.
    $|\Delta\delta|$, $|\Delta\tau|$ are the mean steering/throttle corrections.}
  \label{tab:convoy_cf}
  \centering
  \setlength{\tabcolsep}{3pt}
  \footnotesize
  \begin{tabular}{lccccc}
    ...
    \midrule
    None      & $12/15$ & --- & $-1.42$ & --- & ---\\
    \textbf{DOB-CBF} & $\mathbf{2/15}$ & $\mathbf{10/12}$ & $\mathbf{+1.82}$ & $0.13$ & $0.26$\\
    \bottomrule
  \end{tabular}
\end{table}
...
profile with and without the safety filter:
Table~\ref{tab:latency_comp} reports the result.
...
about half of the steps --- the minimum-deviation, intent-preserving
behaviour that suits the human-in-the-loop case (Sec.~\ref{sec:safety})
holding up even under the profile's outage spikes.
...
\begin{table}[!t]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    None      & $2.85$ & $-1.48$ & ---\\
    \textbf{DOB-CBF}   & $\mathbf{0.39}$ & $\mathbf{+0.35}$ & $51$\\
    \bottomrule
  \end{tabular}
\end{table}
...
re-using the neural surrogate, and a deterministic counterfactual replay
attributes the prevented collisions causally to the filter on the
operator's own intent ($10$ of $12$ averted) --- robust to the
inter-operator variance that makes raw teleoperation data noisy. The forward
