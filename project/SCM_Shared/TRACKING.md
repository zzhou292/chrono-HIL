...
## 2026-04-20: SCM_Shared packaging + verification

Created `project/SCM_Shared/` as a curated paper bundle extracted from
`SCM_Teleop/`.

Included in the shared bundle:

* active runtime under `simulation/`
* active paper scripts under `utilities/` and `test_suite/`
* active checkpoints under `nn_models/`
* terrain-estimator / residual datasets under `data/`
* paper assets under `my_paper/`

Deliberately omitted from the copy:

* the source repo's large `simulation/plots/`
* historical archives not needed for the active paper path

Bundle-specific organization changes:

* added `verification/` with a reusable shared-bundle verification script
* added `.gitignore` and a shared-bundle README
* moved copied classifier training corpora into
  `archive/optional_classifier_training/` to keep the active tree focused

Verification executed from inside `SCM_Shared/`:

1. `python -m compileall simulation test_suite utilities my_paper`
   * passed
2. `python test_suite/eval_learned_estimator.py`
   * canonical late-window replay error:
     clay `0.9%`, dirt `0.8%`, sand `0.6%`
   * expected larger misses remain on some off-manifold variants
3. `python test_suite/validate_closed_loop_estimator.py --terrains clay dirt sand --duration 20`
   * clay: `n_mean=0.5038`, `|err|=0.0038`
   * dirt: `n_mean=0.6990`, `|err|=0.0010`
   * sand: `n_mean=1.0930`, `|err|=0.0070`
   * outputs regenerated:
     `my_paper/paper_figures/closed_loop_estimator_learned.png`
     `my_paper/paper_figures/closed_loop_estimator_summary_learned.csv`
4. `python utilities/generate_random_terrains.py --n-terrains 2 --seed 42`
   plus
   `python test_suite/validate_random_terrains_closed_loop.py --terrains terrain1 terrain2 --duration 20`
   * terrain1: true `n=0.8837`, estimated `0.8935`, `|err|=0.0098`,
     RMS CTE `0.035 m`
   * terrain2: true `n=0.6339`, estimated `0.6937`, `|err|=0.0598`,
     RMS CTE `0.091 m`
   * outputs regenerated:
     `my_paper/paper_figures/random_terrain_closed_loop_learned.png`
     `my_paper/paper_figures/random_terrain_closed_loop_learned.csv`
5. `python utilities/bench_tire_models.py --workers 2 --repeats 1 --models Pacejka TMeasy MLP`
   * required a robustness fix in the shared bundle so child runs use
     `conda run -n sim`
   * regenerated:
     `my_paper/paper_figures/bench_tire_models.csv`
     `my_paper/paper_figures/bench_tire_models_summary.csv`
     `my_paper/paper_figures/bench_tire_models.png`
   * compact benchmark findings:
     MLP stayed strong across the sampled scenarios
     (`0.020 m` sand lane-change, `0.058 m` sand sinusoidal,
      `0.095 m` dirt sinusoidal, `0.260 m` clay sinusoidal)
     while TMeasy degraded badly on sinusoidal clay/dirt
     (`2.475 m`, `1.284 m`)
   * one Pacejka clay-sinusoidal run timed out at `240 s`
6. `python utilities/exp_joint_n_phi.py --epochs 60 --trace-dir data/terrain_traces data/terrain_traces_rich --yaml-dir data/terrain_yamls data/terrain_yamls_rich`
   * loaded `674` traces / `53,947` windows
   * joint validation:
     `n_rmse=0.0476`, `phi_rmse=3.9145`
   * n-only validation:
     `n_rmse=0.0448`
   * outputs regenerated:
     `my_paper/paper_figures/exp_joint_n_phi.csv`
     `my_paper/paper_figures/exp_joint_n_phi.png`
     `my_paper/paper_figures/exp_joint_n_phi.txt`
7. `python my_paper/make_gp_paper_figure.py`
   * regenerated `my_paper/paper_figures/gp_paper_figure.png`

Conclusion:

* `SCM_Shared/` is now a self-contained paper bundle that can regenerate the
  active terrain-estimation and compact tire-benchmark artifacts directly from
  the shared copy.
* The shared bundle exposed one real maintenance issue: some benchmark drivers
  assumed the caller's Python env. Those were patched in `SCM_Shared/` to
  launch child runs through `conda run -n sim`.
* Runtime validation is therefore positive for the shared bundle, with one
  known benchmark anomaly still present: Pacejka on clay sinusoidal timed out
  in the compact benchmark sweep.
