...
class ReferencePath:
    ...
    def _build_speed_profile(self):
        ...
        # Asymmetric accel/brake: cornering-exit traction is the binding limit
        # on low-friction surfaces, so keep a_accel ≤ a_brake to avoid ramping
        # v_ref back up before the vehicle is fully out of the turn.
        a_accel = 1.5      # m/s² — gentle acceleration out of turns
        ...
        # Speed-aware preview window (≈1 s lookahead at v_target), floored at
        # 3 m so low-speed paths still get curvature anticipation.
        preview_time = 1.0  # s
        preview_dist = max(3.0, preview_time * self.v_target)
        kappa_margin = 1.10  # standard 10% inflation of previewed curvature
