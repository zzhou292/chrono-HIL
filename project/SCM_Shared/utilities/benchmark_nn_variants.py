...
def run_one(model_name: str, terrain: str, bumpiness: int, sim_time: float, speed: float,
    ...
    sim_cmd = [
        sys.executable, str(SIM_DIR / "chrono_sim_node.py"),
        "--time", str(sim_time),
        "--speed", str(speed),
        "--terrain", terrain,
    ...
    ctrl_cmd = [
        sys.executable, str(SIM_DIR / "acados_mpc_controller_node.py"),
        "--model", "nn",
        "--nn-model", model_name,
        "--terrain", terrain,
        "--time", str(sim_time),
...
def main():
    ...
    p.add_argument("--out", default="my_paper/paper_figures/nn_variant_benchmark.csv")
