...
int main(int argc, char *argv[])
{
    ...
    std::cout << "test poin 1 " << std::endl;
    int *vehicle_brake_signal = new int(0);
    // TODO: fix this mother fucking shit
    // Create the input listener for the driver
    // const std::string status_topic_name = "/vehicle_status";
    // auto status_handler =
    ...
    tf_handler->AddSensor(driver_cam, "world", "camera");
    tf_handler->AddSensor(lidar, "world", "lidar");
    ros_manager->RegisterHandler(tf_handler);

    std::cout << "test poin 3 " << std::endl;
    // Finally, initialize the ros manager
    ros_manager->Initialize();
    std::cout << "test poin 4 " << std::endl;
