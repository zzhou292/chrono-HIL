...
::
    ...
    python data_collection/collect_closed_loop_data.py \\
        --scenarios 120 --workers 6 --time 10 --output data/closed_loop_v1

The result is ``data/closed_loop_v1/training_data.csv``,
``data/closed_loop_v1/training_data_tire_frame.csv``, and per-run logs
in ``data/closed_loop_v1/per_run/``.
"""
...
def main():
    ...
    p.add_argument('--nn-model', default='paper_v2_mlp_16_4',
                   help='Bootstrap surrogate for the controller during '
                        'data collection.  The controller still needs '
                        '*some* model to run; this one only shapes the '
                        'distribution of slip/throttle commands we '
                        'observe, not the chrono ground-truth Fy we log.')
