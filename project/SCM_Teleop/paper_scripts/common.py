...
DEFAULT_NN_MODEL = "closed_loop_v2_both_axles_rate_32_16"
...
def find_diag_csv(run_dir: Path, controller_mode: str, created_after: float) -> Path | None:
    if controller_mode == "mpcc":
        direct = run_dir / "diag_mpcc.csv"
        if direct.exists():
            return direct
    candidates = [
...
def parse_diag_csv(path: Path, controller_mode: str, speed: float, metric_start: float = 2.0) -> dict[str, float]:
    ...
    if df.empty:
        return {"n_samples": 0}
    time_col = "time" if controller_mode == "mpcc" else "sim_time"
    t = _float_series(df, time_col)
    ...
    if controller_mode == "mpcc":
        cte = _float_series(df, "e_lat")
        u = _float_series(df, "u")
        solve = _float_series(df, "solve_time_ms")
        x = _float_series(df, "x")
        y = _float_series(df, "y")
        vtheta = _float_series(df, "vtheta_cmd")
        vcap = _float_series(df, "v_max_path")
        theta = _float_series(df, "theta")
        extra = {
            "mean_vtheta_cmd": float(np.nanmean(vtheta[mask])) if vtheta.size == len(df) else math.nan,
            "mean_vmax_path": float(np.nanmean(vcap[mask])) if vcap.size == len(df) else math.nan,
        }
    else:
    ...
    if controller_mode == "mpcc" and theta.size == len(df):
        theta_f = _finite(theta)
        if len(theta_f) >= 2:
            progress = float(theta_f[-1] - theta_f[0])
...
def parse_collision_csv(path: Path | None) -> dict[str, float | int]:
    ...
    for _, row in df.iterrows():
        rid = int(row.get("rock_id", -1))
        if int(row.get("is_collision", 0)) == 1 and rid >= 0:
            hit_ids.add(rid)
        if int(row.get("is_near_miss", 0)) == 1 and rid >= 0:
            near_ids.add(rid)
...
def launch_and_collect(
    *,
    ...
    collision = copy_global_log("collision_log.csv", run_dir, created_after) if rocks > 0 else None
    shield = None
    for name in ("mppi_shield_log.csv", "nmpc_shield_log.csv", "cbf_filter_log.csv"):
        copied = copy_global_log(name, run_dir, created_after)
