...
def extra_args(filter_name: str, delay: float, mpc_delay_comp: bool, args: argparse.Namespace) -> list[str]:
    out: list[str] = ["--mpc-blind-obstacles"]
    if args.latency_profile_json:
        out += ["--latency-profile-json", args.latency_profile_json]
        out += ["--latency-profile-log", str(args._active_run_dir / "latency_profile.csv")]
    elif delay > 0.0:
...
def main() -> None:
    ...
    idx = 0
    total = (
        len(args.filters) * len(args.delays) * len(comp_modes) * len(args.terrains)
                        ...
                        for speed in args.speeds:
                            for bump in args.bumpiness:
                                for seed_i in range(args.seeds):
                                    ...
                                    idx += 1
                                    print(f"[{idx}/{total}] {variant} {terrain}/{path} seed={seed}")
                                    ...
                                    res = launch_and_collect(
                                        ...
                                        extra_args=extra_args(filter_name, delay, mpc_comp, args),
                                    )
                                    res.extra.update({
                                        "filter": filter_name,
                                        "delay_s": delay,
                                        "mpc_delay_comp": "on" if mpc_comp else "off",
                                        "latency_profile": Path(args.latency_profile_json).name if args.latency_profile_json else "",
                                    })
                                    results.append(res)
