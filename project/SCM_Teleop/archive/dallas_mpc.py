...
class NNCasADi:
    ...
    def _load_pytorch_model(self, model_path, scaler_path):
        ...
        nn_training_path = Path(__file__).parent / "nn_training"
    ...
    def _build_casadi_function(self):
        ...
        Feature order for v6 (Dallas format): 
            slip_ratio, slip_angle, velocity, Fz, steering_rate, Kphi, Kc, n, c, phi, k
        """
...
class DallasMPC:
    ...
    def _setup_solver(self):
        ...
        if self.use_nn and self.nn_casadi is not None:
            Fxf_wheel, Fyf_wheel = self.nn_casadi.predict(alpha_f, Fz_f_wheel, u_safe, kappa, n_terrain_sym, delta_dot)
            Fxr_wheel, Fyr_wheel = self.nn_casadi.predict(alpha_r, Fz_r_wheel, u_safe, kappa, n_terrain_sym, delta_dot)
            ...
            kappa_ref = 0.08
            Fxf_traction, _ = self.nn_casadi.predict(0.0, Fz_f_wheel, u_safe, kappa_ref, n_terrain_sym, delta_dot)
            Fxr_traction, _ = self.nn_casadi.predict(0.0, Fz_r_wheel, u_safe, kappa_ref, n_terrain_sym, delta_dot)
        ...
        self.f = ca.Function('f', [z, zeta, n_terrain_sym], [zdot])
        ...
        self.f_Fx = ca.Function('f_Fx', [z, n_terrain_sym], [Fx_traction],
                                ['z', 'n_terrain'], ['Fx_traction'])
        ...
        # Parameters: [z0(8), x_goal, y_goal, psi_goal, v_target, n_terrain,
        #              x_ref(N+1), y_ref(N+1), psi_ref(N+1), v_ref(N+1)]
        n_ref = 4 * (N + 1)  # Reference trajectory
        P = ca.SX.sym('P', nx + 5 + n_ref)
        ...
        for k in range(N):
            ...
            # -----------------------------------------------------------------
            k1 = self.f(zk, uk, n_terrain_param)
            k2 = self.f(zk + dt/2 * k1, uk, n_terrain_param)
            k3 = self.f(zk + dt/2 * k2, uk, n_terrain_param)
            k4 = self.f(zk + dt * k3, uk, n_terrain_param)
            ...
            # -----------------------------------------------------------------
            ax_k = zk[7]
            Fx_avail = self.f_Fx(zk, n_terrain_param)
    ...
    def solve(self, z0, x_ref, y_ref, psi_ref, v_ref, x_goal, y_goal, psi_goal,
        ...
        p = list(z0)
        p += [x_goal, y_goal, psi_goal, v_ref[0], n_terrain]
