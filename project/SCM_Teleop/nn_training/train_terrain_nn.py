#!/usr/bin/env python3
"""
Neural Network Training for SCM Terrain Force Prediction
Trains a neural network to predict tire forces on deformable terrain
Based on Dallas et al. "Terrain Adaptive Trajectory Planning and Tracking on Deformable Terrains"

Architecture: 2 hidden layers (12 neurons, 2 neurons), tanh activation
"""
...
class TerrainNN(nn.Module):
    """
    Neural network for predicting tire forces on deformable terrain.
    Uses tanh activation for twice continuous differentiability (required by IPOPT).
    
    Supports variable-depth architectures via hidden_sizes list.
    Default (legacy): [12, 2]  — kept for loading old checkpoints.
    Recommended:      [64, 32] — much better capacity, still fast in CasADi.
    """
    
    def __init__(self, input_size=11, output_size=2,
                 hidden1_size=12, hidden2_size=2, hidden_sizes=None):
        super(TerrainNN, self).__init__()
        
        if hidden_sizes is not None:
            sizes = hidden_sizes
        else:
            sizes = [hidden1_size, hidden2_size]
...
class TerrainNNTrainer:
    ...
    def __init__(self, data_file, output_dir='nn_models', device='cpu'):
    ...
    def load_and_preprocess_data(self):
        ...
        # Note: Dallas uses 'steering_rate' as key input for dynamic effects!
        ...
        if 'steering_rate' in df.columns:
            # New Dallas et al. format with steering_rate
            input_features = [
    ...
    def train_model(self, n_epochs=1000, batch_size=128, learning_rate=0.01, patience=50):
        ...
        hidden_sizes = getattr(self, 'hidden_sizes', None)
        model = TerrainNN(input_size=input_size, output_size=output_size,
                          hidden_sizes=hidden_sizes).to(self.device)
        logger.info(f"Architecture: {input_size} -> {' -> '.join(map(str, model.hidden_sizes))} -> {output_size}")
        
        # Loss function and optimizer
        # Use L2 regularization (weight decay) similar to Bayesian regularization
...
def main():
    parser = argparse.ArgumentParser(description='Train terrain neural network')
    parser.add_argument('--data', type=str, required=True, help='Path to training data CSV')
    parser.add_argument('--output_dir', type=str, default='../nn_models/new', help='Output directory (default: ../nn_models/new)')
    ...
    parser.add_argument('--hidden', type=int, nargs='+', default=None,
                       help='Hidden layer sizes (e.g., --hidden 64 32). Default: [12, 2] (legacy)')
    
    args = parser.parse_args()
    
    logger.info("="*80)
    logger.info("Terrain Neural Network Training")
    logger.info(f"Data file: {args.data}")
    logger.info(f"Output directory: {args.output_dir}")
    if args.hidden:
        logger.info(f"Hidden layers: {args.hidden}")
    logger.info("="*80)
    
    trainer = TerrainNNTrainer(args.data, args.output_dir, args.device)
    if args.hidden:
        trainer.hidden_sizes = args.hidden
