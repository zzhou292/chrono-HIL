...
and a downstream Model Predictive Path Integral (MPPI) safety
shield track the terrain and compensate time-varying teleoperator
delay in real time.  We simulate an HMMWV on
...
Simulator, NMPC, terrain estimator, and shield exchange data over
low-latency ZMQ; learned 5G traffic profiles~\cite{Choi23}
drive a configurable queue-load delay model on the camera and
human-control paths, not between simulator and controller.
...
the EMA teleoperator round-trip delay, and whose friction-cone
penalty is tightened by the $\phi$ estimate and its ensemble
uncertainty; we benchmark against the prior DOB-CBF
filter~\cite{Zhang25} and a gradient-based predictive NMPC variant
on the same rollout; and (iv)~an open-source parallelized
...
penalty tightened by $\hat\phi - \sigma_\phi$, (c) a terrain-aware
speed cap, and (d) deviation from operator intent so the shield is
minimally invasive when the operator command is already safe.
Hand-crafted seed trajectories (passthrough, full brake, coast,
evade-left/right, brake-while-turn) are injected unconditionally
so the optimizer always has a recoverable option in the sample
set.  A scipy-SLSQP \emph{NMPC} variant on the identical NN-surrogate
rollout serves as the gradient-based ablation; closed-loop tests
on clay show MPPI matches the NMPC variant on collision rate while
running $1.5\times$ faster wall-clock and intervening with
markedly larger control authority via the importance-weighted
action mean.
