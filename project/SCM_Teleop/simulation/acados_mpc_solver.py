...
Usage:
    ...
    delta_dot, Jx, Z_opt, U_opt = mpc.solve(z0, x_ref, y_ref, psi_ref, v_ref,
...
class AcadosDallasMPC:
    ...
    Direct steering-angle control formulation: the steering angle δ is
    a control input (not a state), eliminating the double-integration
    amplification (δ̇ → δ → Fy → ψ̇ → ψ → y) that causes SQP solvers
    to produce bang-bang steering rates.

    State:   z = [x, y, ψ, u, v, ω, ax, δ_prev, Jx_prev]   (nx = 9)
    Control: ζ = [δ, Jx]                                      (nu = 2)

    A linear rate constraint |δ(k) - δ_prev(k)| ≤ max_rate·dt limits
    the effective steering rate.
    """
    ...
    def __init__(self, nn_tire_model=None, dt=0.1, N=20,
        ...
        self.nu = 2    # [δ, Jx]  — steering ANGLE is the control
        ...
        self.max_steer_rate = 0.5  # rad/s — enforced via linear constraint

        # Cost weights
        self.w_lateral = 120.0
        self.w_heading = 20.0
        self.w_speed = 10.0
        self.w_delta_dot = 800.0  # penalises effective steering rate (δ-δ_prev)/dt
        self.w_Jx = 20.0
        self.w_terminal = 50.0
        self.w_du = 500.0         # penalises control-to-control change (Jx only)
        self.w_steer = 0.0
        ...
        # Full SQP (multiple iterations) instead of SQP_RTI (1 iteration).
        # SQP_RTI linearises dynamics once and solves a single QP, which
        # overestimates the sensitivity of lateral position to δ̇ and drives
        # the solution to bang-bang (δ̇ at bounds).  Multiple SQP iterations
        # re-linearise at better operating points, letting the quadratic δ̇
        # penalty produce a smooth interior solution.
        self._lm = 1e-3
        self._qp_iter_max = 50
        self._nlp_max_iter = 5
    ...
    def _build_ocp(self):
        ...
        # Control bounds: δ ∈ [δ_min, δ_max], Jx ∈ [Jx_min, Jx_max]
        ocp.constraints.lbu = np.array([self.delta_min, self.Jx_min])
        ocp.constraints.ubu = np.array([self.delta_max, self.Jx_max])
        ocp.constraints.idxbu = np.array([0, 1])

        # Steering rate constraint: -max_rate*dt ≤ δ - δ_prev ≤ max_rate*dt
        _max_dd = self.max_steer_rate * dt
        ocp.constraints.C = np.zeros((1, nx))
        ocp.constraints.C[0, 7] = -1.0       # -δ_prev
        ocp.constraints.D = np.zeros((1, nu))
        ocp.constraints.D[0, 0] = 1.0        # +δ
        ocp.constraints.lg = np.array([-_max_dd])
        ocp.constraints.ug = np.array([+_max_dd])
    ...
    def _build_acados_model(self) -> AcadosModel:
        ...
        # Unpack controls — δ is now a control input, not a state
        delta = u_ctrl[0]
        Jx = u_ctrl[1]

        # ---- Tire force computation ----
        u_safe = ca.fmax(ca.fabs(u_vel), 0.5)
        alpha_f = delta - ca.atan2(v_vel + Lf * omega, u_safe)
        ...
        # Unpack augmented state: previous controls (for du/smoothness penalty)
        delta_prev = x[7]
        Jx_prev = x[8]

        # ---- Dynamics: ẋ = f(x, u) ----
        # δ is a control input (constant during each stage).  The dynamics
        # don't integrate δ̇.  States 7-8 track previous controls: Euler
        # integration of (u - u_prev)/dt gives u_prev(k+1) = u(k).
        _dt = self.dt
        f_expl = ca.vertcat(
            ...
            Jx,                                                          # ȧx
            (delta - delta_prev) / _dt,                                  # δ_prev → δ(k)
            (Jx - Jx_prev) / _dt,                                       # Jx_prev → Jx(k)
        )
        ...
        # Effective steering rate penalty: (δ - δ_prev)² ∝ (δ̇·dt)²
        # Scale by 1/dt to match the original w_delta_dot·dt·δ̇² formulation.
        d_delta = delta - delta_prev
        d_Jx = Jx - Jx_prev

        stage_cost = (
            self.w_delta_dot / _dt * d_delta**2 +
            self.w_Jx * _ct * Jx**2 +
            self.w_du * d_Jx**2 +
    ...
    def solve(self, z0, x_ref, y_ref, psi_ref, v_ref,
        ...
        if _have_prev:
            ...
            for k in range(N):
                xk, yk, psik, uk, vk, omegak, axk = zk[:7]
                delta_prev_k, jerk_prev_k = zk[7], zk[8]
                psi_err = np.arctan2(np.sin(psi_ref[k] - psik),
                                     np.cos(psi_ref[k] - psik))
                y_err = (y_ref[k] - yk) * np.cos(psik) - (x_ref[k] - xk) * np.sin(psik)
                delta_k = float(np.clip(psi_err * 1.5 + y_err * 0.2,
                                        self.delta_min, self.delta_max))
                # Rate-limit the desired angle
                delta_k = float(np.clip(delta_k,
                                        delta_prev_k - self.max_steer_rate * _dt,
                                        delta_prev_k + self.max_steer_rate * _dt))
...
if __name__ == '__main__':
    ...
    z0 = np.zeros(mpc.nx)
    z0[:3] = [x_ref[0], y_ref[0], psi_ref[0]]
    z0[3] = v_target

    # Warm up
    for _ in range(5):
        dd, jx, Z, U = mpc.solve(z0, x_ref, y_ref, psi_ref, v_ref,
