...
NN_MODELS_ROOT = SCRIPT_DIR.parent / "nn_models"
...
def run_one(job: dict) -> dict:
    ...
    cmd = [
        sys.executable,
        str(SCRIPT_DIR / "launch_decoupled.py"),
        "--model",
        "nn",
        "--nn-model",
        nn_model,
    ...
    t0 = time.time()
    try:
        proc = subprocess.run(cmd, timeout=420, cwd=str(SCRIPT_DIR))
        wall = time.time() - t0
...
def main():
    ...
    out_path = SCRIPT_DIR / "mpc_benchmark_results.json"
