...
Runs three comparisons across all terrains × paths:
  1. Analytical models (pacejka, tmeasy, linear) vs best NN (paper_v1_mlp_16_4)
  2. All working static NN models (MLPs + ResNets)
  3. All NN model types including temporal/rate (solver success comparison)

Each (model, terrain, path) combination runs once.  Results are collected
from per-run diagnostic CSVs and written to a single JSON + summary CSV.

Usage:
    # Full suite (all 3 comparisons, all terrains × paths)
...
# Comparison 3: ALL v1 types including temporal/rate (solver success table)
ALL_NN_MODELS = {
    # Static MLP
    "mlp_12_2":             "paper_v1_mlp_12_2",
    "mlp_16_4":             "paper_v1_mlp_16_4",
...
def run_one(job):
    ...
    tag = f"{label}__{terrain}__{path_name}"
    ...
    if not run_dir:
        ...
        if stderr_tail:
            print(f"    stderr: {stderr_tail[:200]}")
        return {"label": label, "terrain": terrain, "path": path_name,
                "status": "no_output", "wall_s": round(wall, 1)}
...
# ── Build job list ────────────────────────────────────────────────────
def build_jobs(suites, speed, sim_time, lead_in, base_port):
    """Build flat list of jobs for the requested suites."""
    jobs = []
    seen = set()

    def _add(label, terrain, path_name, path_args, model_cfg):
        key = (label, terrain, path_name)
        if key in seen:
            return
        seen.add(key)
        jobs.append({
        ...
        for name, cfg in ANALYTICAL_MODELS.items():
            for terrain in TERRAINS:
                for pname, pargs in PATHS.items():
                    _add(name, terrain, pname, pargs, cfg)
...
def write_summary_csv(results, out_path):
    ...
    with open(out_path, "w", newline="") as f:
        ...
        w.writeheader()
        for r in sorted(results, key=lambda x: (x["label"], x["terrain"], x["path"])):
            w.writerow(r)
...
def print_comparison_table(results, title, group_by="label"):
    ...
    header = f"{'Model':<22s} {'Terrain':<6s} {'Path':<20s} {'RMS CTE':>8s} {'Hdg°':>6s} {'Solver%':>8s} {'ms':>6s} {'Spd%':>5s}"
    print(header)
    print("-" * len(header))

    for model in sorted(grouped.keys()):
        runs = sorted(grouped[model], key=lambda x: (x["terrain"], x["path"]))
        for r in runs:
            ...
            print(f"{model:<22s} {r['terrain']:<6s} {r['path']:<20s} {rms:>8s} {hdg:>6s} {sol:>8s} {ms:>6s} {spd:>5s}{stall}")
...
def main():
    ...
    jobs = build_jobs(args.suite, args.speed, args.time, args.lead_in,
                      args.base_port)
    ...
    if args.dry_run:
        for j in jobs:
            print(f"  {j['label']:22s}  {j['terrain']:6s}  {j['path_name']:20s}  "
                  f"ports {j['sim_port']}/{j['ctrl_port']}")
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            for fut in concurrent.futures.as_completed(futures):
                try:
                    ...
                    job = futures[fut]
                    print(f"  EXCEPTION {job['label']}/{job['terrain']}/{job['path_name']}: {e}")
                    results.append({
                        "label": job["label"],
                        "terrain": job["terrain"],
                        "path": job["path_name"],
