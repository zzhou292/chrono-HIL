...
def main():
    ...
    def _is_held(tr) -> bool:
        ...
        if p_lo is not None and not (p_lo <= tr[4] <= p_hi):
            return False
        # If a range is specified and we made it here, it's in the box.
        if n_lo is not None or p_lo is not None:
            return True
        return False

    if held_substrs or args.holdout_n_range or args.holdout_phi_range:
        ...
        print(f"[exp] hold-out: {len(held)} traces "
              ...
              f"phi_range={args.holdout_phi_range}), train: {len(train)}")
    else:
        held = []
        train = traces
    ...
    summary = "\n".join(lines)
    print("\n" + summary)
    (OUT_DIR / "exp_joint_n_phi.txt").write_text(summary + "\n")

    # ── compact CSV ───────────────────────────────────────────────────
    import csv
    with open(OUT_DIR / "exp_joint_n_phi.csv", "w", newline="") as f:
        w = csv.writer(f)
            ...
            for split in ("val", "holdout"):
                ...
                for tgt, v in Rs["per_target"].items():
                    ...
                    w.writerow([name, split, tgt,
                                ...
                                n_win])
    print(f"[exp] CSV → {OUT_DIR / 'exp_joint_n_phi.csv'}")
    ...
    fig.savefig(OUT_DIR / "exp_joint_n_phi.png", dpi=180,
                bbox_inches="tight")
    print(f"[exp] figure → {OUT_DIR / 'exp_joint_n_phi.png'}")
